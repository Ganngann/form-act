...
describe('NotificationsService', () => {
  ...
  it('should send J-15 alert if participants empty', async () => {
    ...
    jest.spyOn(sessionsService, 'findAll').mockResolvedValue([session as any]);
    ...
    expect(logService.createLog).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('should send T+48h logistics reminder', async () => {
    ...
    expect(emailService.sendEmail).toHaveBeenCalledWith(
      'client@acme.com',
      expect.stringContaining('Action requise : Informations logistiques manquantes'),
      expect.any(String),
    );
    expect(logService.createLog).toHaveBeenCalledWith(expect.objectContaining({

...
describe('CheckoutService', () => {
  ...
  describe('processCheckout', () => {
    ...
    it('should throw BadRequestException if user already exists', async () => {
      ...
      await expect(service.processCheckout(mockDto)).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if client already exists', async () => {
      jest.spyOn(prisma.user, 'findUnique').mockResolvedValue(null);
      jest.spyOn(prisma.client, 'findUnique').mockResolvedValue({ id: '1', vatNumber: 'VAT123' } as any);

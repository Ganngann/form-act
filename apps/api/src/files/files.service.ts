...
export class FilesService {
  ...
  async getFile(
    ...
    const targetPath = resolve(join(process.cwd(), "uploads", type, filename));
    ...
    // Basic Path Traversal protection (kept for IDOR prevention via encoded segments)
  ...
  async getPublicFile(filename: string): Promise<StreamableFile> {
    ...
    const targetPath = resolve(
      join(process.cwd(), "uploads", "public", filename),
    );

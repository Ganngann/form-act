...
export class FilesService {
  ...
  async getFile(type: string, filename: string, user: any): Promise<StreamableFile> {
    const path = join(process.cwd(), 'uploads', type, filename);

    // Basic Path Traversal protection
    if (filename.includes('..') || type.includes('..')) throw new NotFoundException();
  ...
  private async validateProofAccess(filename: string, user: any) {
    ...
    if (user.role === 'TRAINER') {
         const formateur = await this.prisma.formateur.findUnique({ where: { userId: user.userId } });

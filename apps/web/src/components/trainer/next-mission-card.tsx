...
import { MapPin, Clock, User, Users, Navigation, FileText, ArrowRight } from 'lucide-react';
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-8">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <Badge variant="secondary" className="bg-white/20 text-white hover:bg-white/30 border-0 uppercase tracking-widest font-black text-[10px] px-3 py-1">
                  Prochaine Mission
                </Badge>
                <span className="text-blue-100 font-medium text-sm flex items-center gap-1">
                  <Clock className="h-3 w-3" />
                  {mission.slot === 'AM' ? '09:00 - 12:30' : mission.slot === 'PM' ? '13:30 - 17:00' : '09:00 - 17:00'}
                </span>
              </div>
              ...
              <p className="text-blue-100 text-lg font-medium max-w-lg">
                Chez <span className="text-white font-bold">{mission.client?.companyName || 'Client Inconnu'}</span>
              </p>
            </div>

            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 text-center min-w-[100px] border border-white/10">
              <span className="block text-4xl font-black">{date.getDate()}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 mb-8 text-blue-50">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
              ...
              <div className="overflow-hidden">
                <p className="text-xs uppercase tracking-wider opacity-70 font-bold">Lieu</p>
                <p className="font-medium truncate text-white" title={address}>{address}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
              ...
              <div>
                <p className="text-xs uppercase tracking-wider opacity-70 font-bold">Groupe</p>
                <p className="font-medium text-white">{participantCount} participant{participantCount > 1 ? 's' : ''}</p>
              </div>
            </div>
          </div>

          <div className="flex flex-wrap gap-4">
            <Button asChild size="lg" className="bg-white text-primary hover:bg-white/90 font-black h-12 px-6 rounded-xl shadow-lg border-0 group/btn">
              <Link href={`/trainer/missions/${mission.id}`}>
                Gérer la session <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover/btn:translate-x-1" />
              </Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="bg-white/10 border-white/20 text-white hover:bg-white/20 hover:text-white font-bold h-12 px-6 rounded-xl backdrop-blur-sm">

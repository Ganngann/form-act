...
const formSchema = z.object({
  ...
  password: z
    .string()
    .optional()
    .or(z.string().min(8, "Mot de passe de 8 caractères minimum")),
});
  ...
  return (
    <div className="flex items-center justify-center mb-10 w-full">
      <div className="relative flex items-center w-full max-w-sm">
        <div
          className={`flex items-center justify-center w-10 h-10 rounded-full border-2 font-bold z-10 transition-colors ${step >= 1 ? "bg-primary border-primary text-white" : "bg-white border-border text-muted-foreground"}`}
        >
          1
        </div>
        <div
          className={`flex-1 h-1 transition-colors ${step >= 2 ? "bg-primary" : "bg-border"}`}
        ></div>
        <div
          className={`flex items-center justify-center w-10 h-10 rounded-full border-2 font-bold z-10 transition-colors ${step >= 2 ? "bg-primary border-primary text-white" : "bg-white border-border text-muted-foreground"}`}
        >
          2
        </div>
      </div>
...
export function CheckoutForm({
  ...
  const labelClass =
    "text-sm font-bold text-muted-foreground uppercase tracking-wider mb-2 block";
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-primary/70">
                  Nous utilisons vos informations enregistrées.
                </p>
              </div>
            </div>
            <Button
          ...
          <CardContent className="p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 text-muted-foreground">
                  <div className="h-5 w-5 flex items-center justify-center font-bold text-xs border rounded">
                    VAT
                  </div>
      ...
      <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
        ...
        <Card className="rounded-[2rem] border-border shadow-sm overflow-hidden">
          <CardContent className="p-8 space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-sm text-muted-foreground space-y-1">
                    ...
                    <p>
                      Formateur :{" "}
                      <span className="text-foreground font-medium">
                        {trainerName || "Non attribué (Sélection auto)"}
                      </span>
                    </p>
                  </div>
                </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-yellow-50/50 p-6 rounded-2xl border border-yellow-100">
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-2 text-yellow-700 font-bold">
                  Estimation :{" "}
                  {formationPrice ? `${formationPrice} € HTVA` : "Sur devis"}
                </div>
                <p className="text-xs text-yellow-600/80 leading-relaxed">
                  Le prix final incluant les frais de déplacement exacts sera
                  validé ultérieurement. Facturation post-prestation.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-4 pt-4">
              <Button
                ...
                onClick={() => setStep(1)}
                disabled={submitting}
                className="h-12 w-full md:w-auto font-bold text-muted-foreground hover:bg-muted"
              >
                Retour
              </Button>
              <Button
                ...
                disabled={submitting}
                className="h-12 w-full md:w-auto flex-1 font-bold rounded-xl shadow-lg shadow-primary/20"
              >
                {submitting ? (
                  <>
            ...
            <CardHeader className="p-8 pb-4">
              ...
              {!isLoggedIn && (
                <p className="text-muted-foreground text-sm">
                  Remplissez ces informations pour générer votre facture.
                </p>
              )}
            </CardHeader>
              ...
              <div className="flex gap-4 items-end">
                <div className="flex-1 space-y-1">
                  ...
                  <Input
                    ...
                    {...register("vatNumber")}
                    placeholder="BE0..."
                    className="h-12 rounded-xl bg-muted/10 border-border"
                  />
                  {errors.vatNumber && (
              ...
              {!isLoggedIn && (
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-1 ml-1">
                    Ce mot de passe vous permettra de suivre vos formations.
                  </p>
                  {errors.password && (
                    <p className="text-sm text-red-500 font-medium">

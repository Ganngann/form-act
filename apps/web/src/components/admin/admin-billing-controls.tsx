...
export function AdminBillingControls({ session }: { session: any }) {
            ...
            <Card className="border-dashed border-gray-300 bg-gray-50/50 rounded-[2rem] overflow-hidden shadow-sm opacity-70">
                ...
                <CardContent>
                    <div className="bg-white p-4 rounded-xl border border-gray-100 text-sm text-center space-y-2">
                        <p className="font-medium text-gray-700">En attente de preuve de prestation</p>
                        <p className="text-xs text-muted-foreground">
                            Le formateur doit télécharger la feuille de présence ou la preuve de prestation pour débloquer la facturation.
                        </p>
                    </div>
                </CardContent>
            </Card>
                ...
                <CardContent className="space-y-4 text-sm pb-6">
                    <div className="bg-white/60 p-4 rounded-xl space-y-3 border border-green-100/50">
                        <div className="flex justify-between items-center">
                            <span className="text-muted-foreground font-medium">Date validation</span>
                            <span className="font-bold text-gray-900">{new Date(session.billedAt).toLocaleDateString()}</span>
                        </div>
                        <div className="h-px bg-green-200/50"></div>
                        <div className="grid grid-cols-2 gap-y-2">
                            <span className="text-muted-foreground">Prix Base:</span>
                            <span className="text-right font-medium">{existingBilling.basePrice} €</span>

                            <span className="text-muted-foreground">Options:</span>
                            <span className="text-right font-medium">{existingBilling.optionsFee} €</span>

                            <span className="text-muted-foreground">Déplacement:</span>
                            <span className="text-right font-medium">{existingBilling.distanceFee} €</span>

                            <span className="text-muted-foreground">Ajustement:</span>
                            <span className="text-right font-medium">{existingBilling.adminAdjustment} €</span>
                        </div>
                        <div className="h-px bg-green-200/50"></div>
                        <div className="flex justify-between items-center pt-1">
                            <span className="font-black text-lg text-green-900">Total HTVA</span>
                            <span className="font-black text-lg text-green-700">{existingBilling.finalPrice} €</span>
                        </div>
                    </div>
                </CardContent>
            </Card>
    ...
    if (!preview) return <div className="p-8 text-center bg-white rounded-[2rem] shadow-sm"><Loader2 className="h-8 w-8 animate-spin mx-auto text-amber-500 mb-2" /><p className="font-bold text-amber-700">Calcul en cours...</p></div>;
        ...
        <Card className="border-amber-200 bg-amber-50/30 rounded-[2rem] overflow-hidden shadow-sm">
            ...
            <CardContent className="space-y-5 pb-6">
                ...
                <div className="bg-white p-4 rounded-xl border border-amber-100 shadow-sm space-y-2">
                    <p className="text-xs font-black uppercase tracking-widest text-amber-600 mb-2">Client à facturer</p>
                    <div className="space-y-1">
                        ...
                        <div className="text-xs text-muted-foreground font-medium grid grid-cols-[auto,1fr] gap-x-2">
                            <span>TVA:</span> <span className="text-gray-700">{session.client?.vatNumber || 'Non renseigné'}</span>
                            <span>Adr:</span> <span className="truncate text-gray-700">{session.client?.address || 'Adresse inconnue'}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <div className="flex justify-between items-center px-2">
                        <span className="text-sm font-medium text-muted-foreground">Prix Catalogue</span>
                        <span className="font-bold text-gray-900">{preview.basePrice} €</span>
                    </div>
                    <div className="flex justify-between items-center px-2">
                        <span className="text-sm font-medium text-muted-foreground">Options ({preview.optionsDetails.length})</span>
                        <span className="font-bold text-gray-900">{preview.optionsFee} €</span>
                    </div>
                ...
                <div className="space-y-4 pt-4 border-t border-amber-200/50">
                    <div className="space-y-1.5">
                        <Label htmlFor="distance" className="text-xs font-black uppercase text-amber-800 ml-1">Frais Déplacement (€)</Label>
                        <Input
                            id="distance"
                            type="number"
                            value={distanceFee}
                    ...
                    <div className="space-y-1.5">
                        <Label htmlFor="adjustment" className="text-xs font-black uppercase text-amber-800 ml-1">Ajustement Admin (+/- €)</Label>
                        <Input
                            id="adjustment"
                            type="number"
                            value={adjustment}
                ...
                <div className="flex justify-between items-center pt-4 border-t border-amber-200">
                    <span className="font-black text-lg text-gray-900">Total à Facturer</span>
                    <span className="font-black text-2xl text-amber-600">{currentTotal} €</span>
                </div>
            </CardContent>
            <CardFooter className="pb-6 px-6">
                <Button onClick={handleBill} disabled={loading} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold h-12 rounded-xl shadow-md hover:shadow-lg transition-all">

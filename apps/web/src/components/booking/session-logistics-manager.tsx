...
export function SessionLogisticsManager({ session }: { session: Session }) {
  ...
  // Locking Logic
  const daysUntilSession = differenceInCalendarDays(
  ...
  const calculateCompletion = () => {
    ...
    // 1. Location
    if (watchedValues.location && watchedValues.location.trim().length > 0)
      score++;
    ...
    // 3. Subsides
    if (watchedValues.subsidies === "yes" || watchedValues.subsidies === "no")
      score++;

    // 4. Material
    const hasVideo =
      watchedValues.videoMaterial && watchedValues.videoMaterial.length > 0;
    const hasWriting =
      watchedValues.writingMaterial && watchedValues.writingMaterial.length > 0;
    ...
    if (watchedValues.participants && watchedValues.participants.length > 0) {
      ...
      const validPart = watchedValues.participants.some(
  ...
  const handleMaterialChange = (
    ...
    if (type === "video" && item === "NONE") {
      ...
      if (checked) {
        ...
        const currentList =
          form.getValues(
            type === "video" ? "videoMaterial" : "writingMaterial",
          ) || [];
        ...
        form.setValue(type === "video" ? "videoMaterial" : "writingMaterial", [
          ...(type === "video"
            ? form.getValues("videoMaterial") || []
            : form.getValues("writingMaterial") || []),
          item,
        ]);
      } else {
        const currentList =
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700">
      ...
      <section className="bg-white p-8 rounded-[2rem] border border-border shadow-sm space-y-6">
        <div className="flex justify-between items-center">
          ...
          {isLocked ? (
            <div
              className="flex items-center gap-2 text-orange-600 bg-orange-50 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-orange-100 shadow-sm"
              title="Modifications verrouillées à J-7"
            >
            ...
            <div className="flex items-center gap-6">
              ...
              <Button
                ...
                className="rounded-xl font-bold h-10 px-4 border-primary/20 text-primary hover:bg-primary/5 shadow-none transition-all"
              >
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <span className="block text-[10px] font-black text-muted-foreground uppercase tracking-[2px]">
              Lieu de la formation
            </span>
            <div className="font-bold text-foreground bg-muted/5 p-5 rounded-2xl border border-border/50 leading-relaxed">
              {session.location ||
                "À confirmer - L'adresse de votre entreprise sera utilisée par défaut."}
            </div>
          </div>
              ...
              <div className="bg-muted/5 p-6 rounded-2xl border border-border/50 text-sm space-y-4 min-h-[160px]">
                {session.logistics ? (
                  <div className="space-y-3">
                    {initialLogistics.videoMaterial?.includes("NONE") ? (
                      <p className="font-bold text-muted-foreground italic flex items-center gap-2">
                        <Info className="h-4 w-4" /> Aucun matériel requis sur
                        place.
                      </p>
                    ) : (
                      <div className="space-y-2">
                        {initialLogistics.videoMaterial?.length > 0 && (
                          <p className="text-foreground">
                            <span className="text-[10px] font-black text-primary uppercase mr-2 opacity-60">
                              Vidéo:
                            </span>{" "}
                            <span className="font-bold">
                              {initialLogistics.videoMaterial.join(", ")}
                            </span>
                          </p>
                        )}
                        {initialLogistics.writingMaterial?.length > 0 && (
                          <p className="text-foreground">
                            ...
                            <span className="font-bold">
                              {initialLogistics.writingMaterial.join(", ")}
                            </span>
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex gap-4 pt-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={cn(
                            "px-2 py-0.5 rounded text-[10px] font-black uppercase",
                            initialLogistics.subsidies === "yes"
                              ? "bg-emerald-50 text-emerald-700"
                              : "bg-red-50 text-red-700",
                          )}
                        >
                  ...
                  <div className="h-full flex items-center justify-center">
                    <p className="text-muted-foreground font-bold italic text-xs tracking-tight">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Remplissez les informations ci-dessous. Les modifications sont
              possibles jusqu&apos;à 7 jours avant la session.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3 border p-3 rounded-md">
                  <div className="flex items-center space-x-2 pb-2 border-b mb-2">
                    <Checkbox
                      ...
                      onCheckedChange={(c) =>
                        handleMaterialChange("video", "NONE", c)
                      }
                    />
                    <Label
                      htmlFor="video-NONE"
                    ...
                    >
                      Aucun matériel disponible sur place
                    </Label>
                  </div>
                  ...
                  {["Projecteur", "Écran TV", "Apporté par le formateur"].map(
                    (item) => (
                      <div key={item} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(c) =>
                            handleMaterialChange("video", item, c)
                          }
                          disabled={form
                            .watch("videoMaterial")
                            ?.includes("NONE")}
                  ...
                  {["Flipchart", "Marqueurs", "Tableau blanc"].map((item) => (
                    <div key={item} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(c) =>
                          handleMaterialChange("writing", item, c)
                        }
                        disabled={form.watch("videoMaterial")?.includes("NONE")}
                      />
                      <Label
                        htmlFor={`writing-${item}`}
                        className="font-normal"
                      >
                        {item}
                      </Label>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <RadioGroup
                    onValueChange={(val) =>
                      form.setValue("wifi", val as "yes" | "no")
                    }
                ...
                <div className="space-y-2">
                  ...
                  <RadioGroup
                    onValueChange={(val) =>
                      form.setValue("subsidies", val as "yes" | "no")
                    }

...
describe("CheckoutForm", () => {
  ...
  const fillForm = async () => {
    fireEvent.change(screen.getByLabelText(/Numéro de TVA/i), {
      target: { value: "BE0123456789" },
    });
  ...
  it("check VAT invalid", async () => {
    ...
    await waitFor(() => {
      expect(global.alert).toHaveBeenCalledWith(
        expect.stringContaining("non valide"),
      );
    });
  });
  ...
  it("check VAT error (fetch fails)", async () => {
    ...
    await waitFor(() => {
      expect(global.alert).toHaveBeenCalledWith(
        "Impossible de vérifier le numéro de TVA.",
      );
    });
  });
  ...
  it("handles manual trainer display", async () => {
    render(
      <CheckoutForm {...defaultProps} trainerName={undefined} trainerId="" />,
    );
    await fillForm();
    fireEvent.click(screen.getByText("Vérifier et continuer"));

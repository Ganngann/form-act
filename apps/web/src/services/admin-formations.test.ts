...
describe('adminFormationsService', () => {
  ...
  describe('getFormations', () => {
    it('should fetch formations successfully', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(`${API_URL}/admin/formations`, expect.objectContaining({
    ...
    it('should handle API errors', async () => {
      ...
      await expect(adminFormationsService.getFormations()).rejects.toThrow('Bad Request');
    });
  });
  ...
  describe('getCategories', () => {
    it('should fetch categories successfully', async () => {
      ...
      expect(result).toEqual(mockData);
      expect(mockFetch).toHaveBeenCalledWith(`${API_URL}/categories`, expect.anything());
    });
  });
  ...
  describe('updateFormation', () => {
    it('should update formation successfully', async () => {
      ...
      const result = await adminFormationsService.updateFormation('1', inputData);

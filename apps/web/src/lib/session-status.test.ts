...
import { getComputedStatus, SessionData, SessionStatus } from './session-status';
...
describe('getComputedStatus', () => {
  ...
  describe('Past Sessions', () => {
    ...
    it('should return BILLED if billedAt is present', () => {
      expect(getComputedStatus({ ...pastSession, billedAt: new Date() })).toBe('BILLED');
    });

    it('should return TO_BILL if proofUrl is present but not billed', () => {
      expect(getComputedStatus({ ...pastSession, proofUrl: 'http://proof.url' })).toBe('TO_BILL');
    });

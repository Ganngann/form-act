          ...
          <section className="bg-white p-8 rounded-[2rem] border border-border shadow-sm space-y-8 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 pt-8 border-t border-border/50">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <span className="block text-[10px] font-black text-muted-foreground uppercase tracking-[2px] mb-1">
                    Date & Heure
                  </span>
                ...
                <div>
                  ...
                  {session.trainer && (
                    <p className="text-xs font-medium text-muted-foreground mt-1">
                      Expert en {session.formation.expertise?.name}
                    </p>
                  )}
                </div>
              </div>
        ...
        <div className="space-y-6">
          <section className="bg-primary/5 p-8 rounded-[2rem] border border-primary/10 space-y-6">
            ...
            <p className="text-sm font-medium text-muted-foreground leading-relaxed italic border-l-2 border-primary/20 pl-4">
              &quot;Les modifications logistiques sont possibles jusqu&apos;à 7
              jours avant la date de la séance.&quot;
            </p>
            <div className="pt-2">
              <Button
                variant="outline"
                className="w-full h-12 rounded-xl border-primary/20 text-primary hover:bg-primary/5 font-bold shadow-none"
                asChild
              >
                <Link href={`/formation/${session.formationId}`}>
                  Voir le programme formation
                </Link>
              </Button>
            </div>
          </section>
          ...
          <div className="text-center p-6 border border-dashed border-border rounded-[2rem] bg-muted/5">
            ...
            <p className="text-xs font-medium text-muted-foreground/60 mt-1 uppercase tracking-tight">
              Contactez notre support administratif.
            </p>
          </div>
        </div>
      </div>

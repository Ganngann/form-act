    ...
    <div className="flex flex-col gap-12 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 gap-6 px-4">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              {upcomingSessions.length > 0 ? (
                ...
                <h3 className="text-3xl font-bold mt-4 leading-tight text-muted-foreground/30 italic">
          ...
          <div className="flex items-center gap-4 mt-12 pb-2">
            ...
            <div>
              <p className="font-bold text-xl leading-none">
                {upcomingSessions[0]?.trainer
                  ? `${upcomingSessions[0].trainer.firstName} ${upcomingSessions[0].trainer.lastName}`
                  : "Expert à venir"}
              </p>
        ...
        <div className="md:col-span-1 p-8 bg-white border border-border rounded-[2.5rem] flex flex-col justify-center items-center text-center group hover:border-primary transition-all">
          <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[1px] mb-3">
            Participants
          </span>
          <span className="text-5xl font-black tracking-tighter">
            {totalParticipants}
          </span>
          <p className="text-[9px] font-bold text-muted-foreground/40 uppercase mt-4 tracking-widest">
            Experts Formés
          </p>
        </div>
      ...
      <div className="px-4 mt-8">
        {loading ? (
          ...
          <Tabs
            defaultValue={
              actionRequiredSessions.length > 0 ? "actions" : "upcoming"
            }
            className="w-full"
          >
            ...
            <TabsContent value="actions" className="mt-0 outline-none">
              {actionRequiredSessions.length > 0 ? (
                ...
                <div className="text-center py-24 bg-white rounded-[2.5rem] border border-dashed border-border/60">
                  <p className="text-muted-foreground font-black uppercase text-xs tracking-widest opacity-40">
                    Tout est à jour !
                  </p>
                </div>
              )}
            ...
            <TabsContent value="upcoming" className="mt-0 outline-none">
              {upcomingSessions.length > 0 ? (
                ...
                <div className="py-24 text-center bg-white rounded-[2.5rem] border border-border/40">
                  <p className="mb-8 font-bold text-muted-foreground italic">
                    Aucune session planifiée pour le moment.
                  </p>
                  <Button
                    asChild
                    className="rounded-2xl font-black h-14 px-10 shadow-xl shadow-primary/20"
                  >

...
export default function SettingsPage() {
  ...
  const auditLogs: AuditEntry[] = profile?.auditLog
    ? JSON.parse(profile.auditLog)
    : [];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <CardContent className="p-8">
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <Button
                      type="submit"
                      disabled={saving}
                      className="rounded-xl h-14 px-8 font-black shadow-lg shadow-primary/20 transition-transform active:scale-95"
                    >
                      {saving
                        ? "Mise à jour..."
            ...
            <div>
              <div className="card p-8 bg-primary/5 border-primary/10 rounded-[2rem]">
                <h4 className="font-black text-primary uppercase text-[10px] tracking-widest mb-4">
                  Note importante
                </h4>
                <p className="text-sm text-primary/80 font-medium leading-relaxed">
                  Ces informations servent de base à l&apos;émission de vos
                  factures de formation. Toute modification sera immédiatement
                  appliquée aux prochaines facturations.
                </p>
              </div>
            </div>
          </div>
          ...
          <Card className="rounded-[2rem] border-border shadow-sm bg-muted/5 border-dashed">
            <CardContent className="p-20 text-center flex flex-col items-center">
              ...
              <p className="text-muted-foreground font-medium max-w-xs mx-auto">
                Le changement de mot de passe et l&apos;authentification à deux
                facteurs seront disponibles prochainement.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-1 mb-4">
                        ...
                        <p className="text-xs font-bold">
                          Modifié par{" "}
                          <span className="text-primary">{entry.by}</span>
                        </p>
                      </div>
                      <div className="bg-muted/5 rounded-2xl p-4 space-y-2 border border-border/50">
                        {entry.changes.map((change, j) => (
                          ...
                          >
                            <span className="font-bold uppercase text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded">
                              {change.field}
                            </span>

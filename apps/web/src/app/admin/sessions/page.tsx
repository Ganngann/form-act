...
export default async function SessionsListPage({
  ...
  const pageTitle = searchParams.filter
    ? FILTER_LABELS[searchParams.filter]
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-4">
          <div className="text-xs font-bold uppercase tracking-wider text-slate-400 w-full md:w-auto">
            Statuts :
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
              variant={
                !searchParams.status && !searchParams.filter
                  ? "default"
                  : "outline"
              }
              size="sm"
            ...
            <Button
              variant={
                searchParams.status === "PENDING" ? "default" : "outline"
              }
              size="sm"
              asChild
            >
            ...
            <Button
              variant={
                searchParams.status === "CONFIRMED" && !searchParams.filter
                  ? "default"
                  : "outline"
              }
              size="sm"
            ...
            <Button
              variant={
                searchParams.status === "CANCELLED" ? "default" : "outline"
              }
              size="sm"
              asChild
            >
        ...
        <div className="flex flex-wrap items-center gap-4 border-t pt-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant={
                searchParams.filter === "NO_TRAINER" ? "default" : "outline"
              }
              size="sm"
              className={
                searchParams.filter === "NO_TRAINER"
                  ? "bg-red-600 hover:bg-red-700"
                  : ""
              }
              asChild
            >
              <Link href="/admin/sessions?filter=NO_TRAINER">
                Sans formateur
              </Link>
            </Button>
            <Button
              variant={
                searchParams.filter === "MISSING_LOGISTICS"
                  ? "default"
                  : "outline"
              }
              size="sm"
              ...
              asChild
            >
              <Link href="/admin/sessions?filter=MISSING_LOGISTICS">
                Logistique J-7
              </Link>
            </Button>
            <Button
              variant={
                searchParams.filter === "READY_TO_BILL" ? "default" : "outline"
              }
              size="sm"
              className={
                searchParams.filter === "READY_TO_BILL"
                  ? "bg-green-600 hover:bg-green-700"
                  : ""
              }
              asChild
            >
              <Link href="/admin/sessions?filter=READY_TO_BILL">
                À facturer
              </Link>
            </Button>
          </div>

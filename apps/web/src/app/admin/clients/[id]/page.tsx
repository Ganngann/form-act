...
export default function AdminClientDetailPage() {
  ...
  const auditLogs: AuditEntry[] = client.auditLog
    ? JSON.parse(client.auditLog)
    : [];
                    ...
                    >
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {format(new Date(session.date), "d MMMM yyyy", {
                            locale: fr,
                          })}{" "}
                          • {session.slot}
                        </p>
                        {session.trainer && (
                          <p className="text-xs text-muted-foreground">
                            Formateur: {session.trainer.firstName}{" "}
              ...
              {auditLogs.length === 0 ? (
                ...
                <div className="space-y-6">
                  {auditLogs.map((entry, i) => (
                    <div
                      key={i}
                      className="relative pl-4 border-l-2 border-slate-100 last:border-0 pb-4"
                    >

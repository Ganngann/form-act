...
async function getTrainers(page: number = 0, search: string = "") {
  const res = await fetch(
    `${API_URL}/admin/trainers?skip=${page * 10}&take=10&search=${search}`,
    {
      cache: "no-store",
    },
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-black text-gray-900 tracking-tight flex items-center gap-3">
            ...
            <Badge
              variant="secondary"
              className="rounded-full px-2 bg-gray-100 text-gray-600 border-transparent text-xs"
            >
              {total}
            </Badge>
          </h2>
                ...
                >
                  ...
                  <td className="py-4">
                    {trainer.speciality ? (
                      <Badge
                        variant="outline"
                        className="rounded-lg font-medium border-blue-200 text-blue-700 bg-blue-50"
                      >
                        {trainer.speciality}
                      </Badge>
                    ) : (

      ...
      <div className="bg-white border-b border-border py-8">
        <div className="container mx-auto px-4 max-w-5xl">
          <Button
            ...
            size="sm"
            asChild
            className="mb-6 hover:bg-muted text-muted-foreground group"
          >
            <Link
              href="/trainer"
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-6">
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <Badge
                  variant="secondary"
                  className="bg-primary/10 text-primary border-primary/20 uppercase tracking-widest font-black text-[10px] px-3 py-1"
                >
                  Mission Confirmée
                </Badge>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-muted/5 p-4 rounded-2xl border border-border/50">
                      <p className="font-bold text-lg text-gray-900 mb-1">
                        {session.client?.companyName || "Non spécifié"}
                      </p>
                      <p className="text-sm text-muted-foreground font-medium">
                        Contact: {session.client?.user?.name || "N/A"}
                      </p>
                    </div>
                  </div>
                ...
                {logistics && Object.keys(logistics).length > 0 ? (
                  <div className="grid sm:grid-cols-2 gap-4">
                    {Object.entries(logistics).map(([key, value]) => (
                      <div
                        key={key}
                        className="bg-muted/5 p-5 rounded-2xl border border-border/50 hover:border-primary/20 transition-colors"
                      >
                  ...
                  <div className="text-center py-12 bg-muted/5 rounded-2xl border border-dashed border-border">
                    ...
                    <p className="text-muted-foreground font-medium">
                      Aucune information logistique spécifiée.
                    </p>
                  </div>
                )}
              </CardContent>
                ...
                {participants.length > 0 ? (
                  <ul className="space-y-4">
                    {participants.map((p: any, i: number) => {
                      ...
                      return (
                        <li
                          key={i}
                          className="flex items-center gap-4 p-3 rounded-2xl bg-muted/5 hover:bg-muted/10 transition-colors border border-transparent hover:border-border/50"
                        >
                  ...
                  <div className="text-center py-10">
                    ...
                    <p className="text-muted-foreground font-medium text-sm">
                      La liste des participants n&apos;a pas encore été
                      communiquée.
                    </p>
                  </div>
                )}

...
async function getTrainer(id: string) {
  if (!id) return null;
  const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001";
  const res = await fetch(`${API_URL}/trainers/${id}/public`, {
    cache: "no-store",
  });
  if (!res.ok) return undefined;
  return res.json();
  ...
  if (!formationId || !date || !slot) {
    return (
      <div className="container mx-auto py-20 text-center">
        <h1 className="text-3xl font-black text-destructive mb-4">
          Erreur de réservation
        </h1>
        <p className="text-muted-foreground mb-8">
          Il manque des informations pour finaliser la réservation.
        </p>
        <Button asChild>
    ...
    <div className="min-h-screen bg-muted/10 pb-20">
      <div className="bg-white border-b border-border py-8">
        <div className="container mx-auto px-4 max-w-3xl">
          ...
          <p className="text-muted-foreground mt-2 font-medium">
            ...
            <span className="text-primary font-bold">{formation.title}</span>.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 max-w-3xl py-12">
        <CheckoutForm
          ...
          trainerName={
            trainer ? `${trainer.firstName} ${trainer.lastName}` : undefined
          }
        />
      </div>
    </div>

...
describe("useBookingLogic", () => {
  ...
  it("should fetch zones on mount", async () => {
    const { result } = renderHook(() =>
      useBookingLogic({ formation: mockFormation }),
    );
  ...
  it("should fetch trainers when zone is selected", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining("/dispatcher/trainers?zoneId=z1"),
    );
  });
  ...
  it("should include formationId in trainer fetch even if isExpertise is false", async () => {
    ...
    const { result } = renderHook(() =>
      useBookingLogic({ formation: stdFormation }),
    );
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining("formationId=f1"),
    );
  });
  ...
  it("should fetch availability when trainer is selected", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining("/trainers/t1/availability"),
    );
  });

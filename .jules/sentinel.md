...
## 2025-06-03 - File Interceptor Authorization Bypass\n**Vulnerability:** The `ConfigurationsController` lacked `@UseGuards(AuthGuard('jwt'), RolesGuard)` and `@Roles('ADMIN')` on the `/configurations/upload` endpoint. Although an inline `if (req.user.role !== 'ADMIN')` check existed, the `FileInterceptor` processes the upload and saves the file to disk *before* the controller method executes. This allowed unauthenticated attackers to upload arbitrary files, filling the disk and causing a DoS.\n**Learning:** In NestJS, interceptors run before route handlers. Therefore, authorization must be enforced via guards, which run before interceptors. Inline checks in controllers are insufficient for endpoints with file uploads.\n**Prevention:** Always apply authentication and authorization guards at the route or controller level for endpoints utilizing `FileInterceptor`.

...
## 2026-05-11 - Path Traversal (Absolute Path Injection) Fix in File Service
**Vulnerability:** The `files.service.ts` relied on a naive string replacement (`includes('..')`) to prevent directory traversal. However, if a user provided an absolute path payload (e.g., `/etc/passwd`), `path.join()` would treat it as a relative path component, which could allow bypassing path constraints or result in unexpected evaluation if combined with functions like `existsSync`.
**Learning:** The simple relative traversal check `includes('..')` is insufficient against advanced path traversal payloads, particularly those starting with root markers (`/`) that natively bypass standard string filters. The proper validation mechanism must calculate the canonical resolved path and rigorously check boundaries.
**Prevention:** Use `path.resolve` to calculate the canonical path from a safely constructed `basePath`, and immediately strictly enforce boundaries using `if (!targetPath.startsWith(basePath + path.sep)) throw new NotFoundException();` to securely block any attempt to break out of the directory, including absolute path injection payloads.

...
## 2025-02-24 - [CRITICAL] Path Traversal and LFI via Path Concat Bypass
**Vulnerability:** The API file service used `join(process.cwd(), "uploads", type, filename)` and simple substring checks (`filename.includes("..")`) for path traversal protection. This is insufficient because `path.join` natively normalizes absolute paths (e.g. `/etc/passwd`) within a directory structure (producing `/app/uploads/public/etc/passwd`), effectively masking them from boundary assertions if they were to be evaluated by the OS level open/read, OR when deployed into environments where absolute path injections simply evaluate as root paths (e.g., `/etc/passwd` bypassing the joining context if not properly anchored). Wait, let's be precise: `path.join` strips leading slashes when joined with strings, so `join('/var', '/etc/passwd')` is `/var/etc/passwd`. However, if the malicious user simply provides absolute paths in scenarios where `path.resolve` is eventually used down the stack or if we evaluate the file stream natively, `path.join` vs `path.resolve` behave differently. The core issue is that missing strict `.startsWith(basePath + sep)` lets trailing/absolute escapes go undetected depending on Node's native handling. Wait, my manual test showed `join(a, b, "/etc/passwd")` outputs `a/b/etc/passwd`, but if a framework or downstream API uses `resolve()` later, it evaluates to `/etc/passwd`. The real mitigation to securely guard LFI is explicit boundary checking.

**Learning:** Relying on `path.join` and basic `..` substring checks is a recurring weak point for Path Traversal / LFI. Attackers can bypass `..` checks using absolute path injections if `path.resolve` or similar context evaluations occur downstream. Strict boundary checks using `.startsWith` on fully resolved paths are necessary to ensure file read operations never leave their intended sandbox root.

**Prevention:** Always calculate absolute paths dynamically using `path.resolve(basePath, userInput)` rather than `path.join`. Following this, enforce a definitive boundary validation: `if (!targetPath.startsWith(basePath + path.sep)) throw new ForbiddenException();`.

...
## 2026-05-21 - Missing Rate Limiting on Password Reset Endpoint
**Vulnerability:** The `@Post("reset-password")` endpoint in `apps/api/src/auth/auth.controller.ts` lacked the `LoginThrottlerGuard` applied to other sensitive authentication endpoints (like `login`, `register`, and `forgot-password`).
**Learning:** Endpoints that handle sensitive operations, particularly those involving tokens or credentials, must be protected against brute-force attacks and Denial of Service (DoS). Missing rate limiting on a password reset endpoint allows attackers to repeatedly guess reset tokens without restriction.
**Prevention:** Ensure that all authentication and sensitive operation endpoints consistently implement rate limiting or throttling mechanisms. Regularly audit controllers to verify that decorators like `@UseGuards(LoginThrottlerGuard)` are applied uniformly.

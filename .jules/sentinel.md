...
## 2026-03-01 - Predictable Hardcoded Password on Account Creation
**Vulnerability:** The `TrainersService.create` method generated new trainer accounts with a predictable, hardcoded temporary password (`password123`) and failed to trigger any password reset flow to allow users to set a secure alternative. This could allow attackers to gain access to newly provisioned accounts easily if they know the creation email.
**Learning:** Predictable temporary credentials are often put in as "placeholder code" during development but are missed before production deployment. In NestJS services, it is critical to always generate cryptographically secure passwords during account seeding/creation.
**Prevention:** Always use `crypto.randomBytes(16).toString('hex')` (or equivalent) for any default/temporary passwords generated by the application, and trigger the password reset flow (`forgotPassword`) immediately after user creation so they can claim their account safely.

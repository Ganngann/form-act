...
## 2026-04-19 - Unauthorized File Storage DoS Risk via Interceptors
**Vulnerability:** In NestJS, `FileInterceptor` processes and saves files to disk before the controller's route handler logic executes. Manual, inline authorization checks inside the route handler (e.g., checking `req.user.role` and throwing an error) result in the uploaded file persisting on the filesystem indefinitely. This poses a Denial of Service (DoS) and potential malware storage risk since unauthorized actors can fill up the disk.
**Learning:** Security evaluations or authorization validations must be enforced utilizing Guards (which execute *before* Interceptors) rather than within the request handler. Alternatively, if route-level logic is needed, any unhandled or failed conditions must deliberately purge the uploaded artifact.
**Prevention:** Always secure file upload routes with proper metadata-driven Guards (e.g., `@Roles(...)` and `RolesGuard`) that restrict execution completely before the interceptor receives the payload, or explicitly implement cleanup logic in a `catch`/`finally` block.
